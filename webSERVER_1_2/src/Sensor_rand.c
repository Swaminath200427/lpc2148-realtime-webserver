...
__declspec(dllexport) double generate_random(double min, double max) {
    return min + (rand() / (double)RAND_MAX) * (max - min);
}

// Function to simulate sensor data generation
__declspec(dllexport) void Getdata(SensorData *data) {
...
__declspec(dllexport) char* sensor_data_to_json(SensorData *data) {

...
#elif defined(__LPC2148__)  // Add a condition for your specific platform
...
struct mg_addr {
    ...
    bool is_ip6;      // True if IPv6 address is used
};
...
//typedef long time_t;  // Use long as a placeholder for time_t on embedded systems
struct mg_fs {
...
PORTABLE_8439_DECL size_t mg_chacha20_poly1305_encrypt(
    uint8_t *PORTABLE_RESTRICT cipher_text, const uint8_t key[RFC_8439_KEY_SIZE],
    const uint8_t nonce[RFC_8439_NONCE_SIZE], const uint8_t *PORTABLE_RESTRICT ad,
    size_t ad_size, const uint8_t *PORTABLE_RESTRICT plain_text, size_t plain_text_size);
...
PORTABLE_8439_DECL size_t mg_chacha20_poly1305_decrypt(
    uint8_t *PORTABLE_RESTRICT plain_text, const uint8_t key[RFC_8439_KEY_SIZE],
    const uint8_t nonce[RFC_8439_NONCE_SIZE],
    const uint8_t *PORTABLE_RESTRICT cipher_text, size_t cipher_text_size);
#if defined(__cplusplus)
}
#endif
#endif
...
typedef void (*mg_event_handler_t)(struct mg_connection *, int ev,
                                   void *ev_data);
void mg_call(struct mg_connection *, int , void *); // *c,ev,*ev_data
